...
所以说yield/send是个伪协程，它只是提供了在yield处保存现场并切换控制权，仍然需要其他函数调用send/nexe来触发，另外它并没有提供并发的能力，主线程需要等待生成器调用，包括IO等待，从结果上看耗时没有任何降低。生成器只能降低内存开销，无法提供并发能力，上面的生产-消费代码也是强耦合，因此Python后续进一步完善，提供了异步IO的能力。async/await代码及结果如下:
